...
class AttachmentsFromInbox implements ShouldQueue
{
    ...
    )
    {
        ...
        $this->pageTokenCacheKey = $this->managerAlias . '_AttachmentsFromInbox_pageToken';
        $this->messagesListCacheKey = $this->managerAlias . '_AttachmentsFromInbox_messagesList';
    }
    ...
    public function handle(Gmail $gmail, Cache $cache)
    {
        ...
        if ( !$file_exists ) {
            $csv->insertRow(['trigger', 'subject', 'email', 'from', 'to', 'emails', 'phones', 'attachments', 'date']);
        }
        ...
        do {
            ...
            foreach ( $this->gmailAPI->messagesTextIterator( $this->messagesList ) as $email_data ) {
                ...
                $csv->insertRow([
                    ...
                    $emails ? implode("\n", $emails) : null,
                    $phones ? implode("\n", $phones) : null,
                    !empty($email_data['attachmentsNames']) ? implode("\n", $email_data['attachmentsNames']) : null,
                    $dateTime
                ]);
